...
Suggested commit checkpoints before pushing:

```bash
git init
git add backend README.md .gitignore
git commit -m "Add Go calculator API"

git add frontend
git commit -m "Add React calculator frontend"

git add docker-compose.yml backend/Dockerfile frontend/Dockerfile PROMPTS.md
git commit -m "Add docs and deployment files"
```
